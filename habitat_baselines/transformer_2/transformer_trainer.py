...
class TransformerTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        self.collect_data_dir = config.RL.TRANSFORMER.COLLECT_DATA_DIR
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            t_obs.append(observations)
            t_act.append(actions.to(device="cpu").numpy())
            t_rwd.append(rewards_l)
            t_dne.append(dones)
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    pickle.dump( [{"obs":t_obs[j][i], "act":t_act[j][i], "rwd":t_rwd[j][i], "dne":t_dne[j][i], "p_obs":t_p_obs[j][i]} for j in range(len(t_obs))], open( self.collect_data_dir+"/1.p", "wb" ) )
